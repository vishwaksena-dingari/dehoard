...
`scree` deletes files, so its design is built around one priority: not deleting the wrong thing.
...
- **Preview by default.** A bare `scree` run, and any run without `--apply`, deletes nothing. You
...
- **Only regenerable data.** scree targets caches, build artifacts, and re-downloadable assets, the
...
So scree grows by demand, not by ambition:
...
scree does not ask you to trust it because it is clever. It earns trust by showing its work: what
